...
class Wpzoom_Feature_Posts extends WP_Widget {
	...
	function Wpzoom_Feature_Posts() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'recent-posts', 'description' => 'A list of posts, optionally filter by category.' );

		/* Widget control settings. */
		$control_ops = array( 'id_base' => 'wpzoom-recent-posts' );

		/* Create the widget. */
		$this->WP_Widget( 'wpzoom-recent-posts', 'WPZOOM: Recent Posts', $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		if ( have_posts() ) : while ( have_posts() ) : the_post();
			echo '<li>';
				...
				if ( $show_date ) echo '<small>' . time_ago() . '</small> <br />';
