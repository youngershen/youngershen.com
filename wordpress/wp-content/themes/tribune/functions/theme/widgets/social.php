		...
		<ul class="social">
			...
			foreach ($socialProfiles as $item)
			{
				...
				if ($instance[$id] && $instance[$id] != '') {
				    ...
				    echo '<li><a class="'.$id.'" href="'.$instance[$id].'" rel="external,nofollow" title="'.$instance[$iditem].'"><img src="'. get_bloginfo('template_directory') .'/images/icons/'.strtolower($item).'.png" alt="'.$instance[$iddesc]. '" />'.$instance[$iditem].'<span>'.$instance[$iddesc].'</a></li>'; 
				}
			} 
			?>
  		</ul>
